    ...
    function InsManufacturer() {
        ...
        if ($("#myModalLabel").html() == "添加生产厂商") {
            $.ajax({
                type: "POST",//这里是http类型
                url: "/EquSpare/InsManufacturer",//大家都应该很清楚了
                async: false,
    ...
    $("#signupForm").validate({
        ...
        rules: {
            manufacturer: {
                ...
                remote: {
                    type: "post",
                    url: "/EquSpare/CheckManufacturer",
                    data: {
                        name: function () {
    ...
    function InsEquhead() {
        ...
        $.ajax({
            type: "POST",//这里是http类型
            url: "/EquSpare/InsQuantityData",//大家都应该很清楚了
            async: false,
            data: "staffname=" + $("#StaffData").val(),//回传一个参数
            dataType: "json",//传回来的数据类型
            success: function (data) {
                if (data.staffname != "") {
                    $("#unit").get(0).options.add(new Option(data.staffname, data.staffname));
                    $("#StaffData").val("");
    ...
    $("#signupForm1").validate({
        ...
        rules: {
            StaffData: {
                ...
                remote: {
                    type: "post",
                    url: "/EquSpare/CheckQuantityname",
                    data: {
                        staffname: function () {
